...
class Leadsource_model extends Model
{
    protected $table      = 'tblleads_source';
    ...
    public function insertLeadsource($data)
    {
        ...
        if (empty($data['lead_source_id'])) {
            ...
            return $this->builder->update($data, [$this->primaryKey => $data['lead_source_id']]);
        }
    }

    var $column_order = array('lead_source_name', null);
    var $column_search = array('lead_source_name');
    var $order = array('lead_source_id' => 'desc');
    ...
    public function deleteLeadsource($ids = null)
    {
        if (!empty($ids)) {
            $this->builder->where('lead_source_id', $ids);
    ...
    public function getLeadsource($dataId = null)
    {
        if (!empty($dataId)) {
            $query = $this->builder->where('lead_source_id', $dataId)->get();
